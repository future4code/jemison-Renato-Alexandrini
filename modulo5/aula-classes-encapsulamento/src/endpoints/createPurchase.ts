...
import { PurchasesClass } from './../classes/Purchases';
...
export const createPurchase = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const newProduct = new ProductClass(findProduct[0].id, findProduct[0].name, findProduct[0].price)
    
        const newPurchase = new PurchasesClass(
           Date.now().toString(),
            userId,
            newProduct.getId(),
            quantity,
            (newProduct.getPrice() * quantity)
           )
